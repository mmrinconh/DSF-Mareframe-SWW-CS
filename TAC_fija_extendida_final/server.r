...
#   }
#   }
PerdidasTrabajodf1<-as.data.frame(as.table(PerdidasTrabajo[11:Anyos,,]))
PerdidasTrabajodfnoNA<-na.omit(PerdidasTrabajodf1)
...
#   }
#   }
perdidasbyship<-split(PerdidasTrabajodfnoNA,PerdidasTrabajodfnoNA$Var3)#Separa por barcos: Tablas con #año,simulacion,barco,valor
for (i in 1:length(Esloras)){
  if(sum(unique(sort(perdidasbyship[[i]]$Freq))-c(0,1))==0){#Pregunta si hay tanto pérdidas y ganancias en la columna
    perdidasbyship_reales<-split(perdidasbyship[[i]],perdidasbyship[[i]]$Freq)#separa por ganancias o pérdidas
    #separa sólo pérdidas=1 por barco
    
    prob2perdu<-which(diff(as.numeric(perdidasbyship_reales[[2]]$Var1))==1 & diff(as.numeric(perdidasbyship_reales[[2]]$Var2))==0)
    #Busca años consecutivos de pérdidas en una simulación
    prob2perd[i]<-length(unique(perdidasbyship_reales[[2]]$Var2[prob2perdu])) #Elimina casos en los que se producen 2 años seguidos de pérdida varias veces en la misma simulación, sólo cuenta 1
  }#prob2perd es el número de simulaciones en las cuales se experimentan dos años seguidos de pérdidas al menos una vez
  else
  {
    
    prob2perd[i]<-perdidasbyship[[i]]$Freq[1]*Simulacion #Como todo son pérdidas o ganancias multiplica 1 o 0 por el número de Simulaciones
  }
}
...
SpawningBiomassMediaPicoPuesta<-apply(SpawningBiomass[11:Anyos,5:7,],c(1,3),mean)
  collapseprob<-length(unique((which(SpawningBiomassMediaPicoPuesta*1e-9<1)-1)%/%(Anyos-10)))/Simulacion #Ojo si hay un colapso en una simulación, no tiene sentido contar más
Tripulantesenriesgomed<-sum(Tripulantes[which(prob2perdporc>50 & Esloras>18 & Esloras<=24)])
#Tripulantes totales flota mediana 221
...
Results2<-cbind(collapseprob, Tripulantesenriesgomed, mean(GananciaPromedioTotal), sd(GananciaPromedioTotal))
...
colnames(Results2)<-c("Probabilidad de colapso (entre 0 y 1)","Trabajadores en riesgo flota Mediana (Total=221 tripulantes), número de tripulantes de barcos con prob. >50% de tener 2 años seguidos de pérdidas","Ganancia media anual de toda la flota (Miles de euros)", "Ganancia sd")
 a<-Results
b<-Results2
s<-53
